...
from pepkio_dose_curve_fitter.config import TOOL_ID
from pepkio_dose_curve_fitter.exceptions import PepkioAPIError
from pepkio_dose_curve_fitter.models import RunResult
...
def test_get_manifest_and_cache(mock_manifest):
    ...
    with _make_client(handler) as client:
        ...
        assert m1["tool_id"] == "dose-curve-fitter"
        assert m1 is m2
    assert len(calls) == 1
...
def test_list_examples_and_get_example_input(mock_manifest):
    ...
    with _make_client(handler) as client:
        ...
        assert inp["ic50_mode"] == "absolute"
        assert len(inp["compounds"]) == 1
...
def test_run_sends_correct_request(mock_manifest, mock_run_response):
    ...
    with _make_client(handler) as client:
        result = client.run(client.get_example_input("hts_batch_absolute"), label="pytest")

    assert captured["path"] == f"/api/tools/v1/tools/{TOOL_ID}/run"
    assert captured["auth"] == "Bearer test-key"
    assert captured["body"]["input"]["model"] == "auto"
    assert captured["body"]["options"]["label"] == "pytest"
    assert result.status == "completed"
    assert result.result["compounds"][0]["qc_grade"] == "green"
...
def test_run_response_error_field(mock_run_response):
    body = {**mock_run_response, "error": {"code": "TOOL_FAILED", "message": "fit failed"}}

    def handler(request: httpx.Request) -> httpx.Response:
        return httpx.Response(200, json=body)

    with _make_client(handler) as client:
        with pytest.raises(PepkioAPIError, match="fit failed"):
            client.run({"model": "four_pl"})
